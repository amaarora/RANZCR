    ...
    def get_score(self, outputs):
        ...
        for i in range(11):
            aucs.append(roc_auc_score(
                y[:, i].cpu().numpy(), y_hat[:, i].cpu().numpy()))
...
if __name__ == '__main__':
    ...
    train_dataloader = torch.utils.data.DataLoader(train_dataset, num_workers=args.num_workers, shuffle=True, batch_size=args.batch_size)
    val_dataloader   = torch.utils.data.DataLoader(val_dataset, num_workers=args.num_workers, shuffle=False, batch_size=args.batch_size)    
    ...
    if args.save_checkpoint:            
        ...
        ckpt = ModelCheckpoint(
            ...
            monitor='macro_auc',
            filename=f'{args.backbone}-fold-{args.fold_id}-{args.img_size}x{args.img_size}'+'-{epoch:02d}-{val_accuracy:.4f}',
            verbose=True, 
            mode='max', 
            period=1, 
    ...
    if args.logger == 'neptune':
        neptune_logger = NeptuneLogger(
            api_key='eyJhcGlfYWRkcmVzcyI6Imh0dHBzOi8vdWkubmVwdHVuZS5haSIsImFwaV91cmwiOiJodHRwczovL3VpLm5lcHR1bmUuYWkiLCJhcGlfa2V5IjoiOWQxZTIzMGUtYzJlYi00NTllLTkyMTEtMDA5MWQ1ODQ3ZmRlIn0=', 
    ...
    trainer = pl.Trainer(
        default_root_dir=ckpt_save_path,
        gpus=args.gpus, 
